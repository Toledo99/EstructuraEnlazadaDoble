...
public class ConjuntoArreglo <T> implements ConjuntoADT <T> {
    ...
    */
    ...
    public ConjuntoADT<T> diferenciaSimetrica(ConjuntoADT<T> otro) {
        if(otro == null || this.cardinalidad==0 || otro.estaVacio())
            throw new NullPointerException("Error: null o vacío.");
        
        ConjuntoADT<T> res = new ConjuntoArreglo<T>();
        ...
        while(it.hasNext()){
            a= it.next();
            i=0;
            while(i<this.cardinalidad && !a.equals(conjunto[i]))
                i++;
            if(i == this.cardinalidad && !res.contiene((T)a))
                res.agrega((T)a);
        }
        return res;
    }
    ...
    public String toString(){
        ...
        for(int i=0; i<this.cardinalidad; i++)
            str.append(conjunto[i].toString()+"\t");
        return str.toString();
    }
    
    public static void main (String[] args){
        ConjuntoADT c1 = new ConjuntoArreglo();
        c1.agrega("A");
        c1.agrega("B");
        c1.agrega("C");
        c1.agrega("D");
        
        ConjuntoADT c2 = new ConjuntoArreglo();
        c2.agrega("G");
        c2.agrega("B");
        c2.agrega("F");
        c2.agrega("D");
        
        System.out.println( (c1.diferenciaSimetrica(c2)).toString() );
